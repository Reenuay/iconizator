...
ipcMain.on("startCleansing", async (e, data) => {
    stopCleansing = false;
    ...
    for (let file of fs.readdirSync(data.iconsFolder)) {
        ...
        jpegs.push(path.join(data.iconsFolder, file));
    }

    maxProgress = jpegs.length;
    ...
    if (await ep.open()) {
        for (const jpeg of jpegs) {
            if (stopCleansing) break;

            const meta = (await ep.readMetadata(jpeg, [
                "Title",
                "Description",
                "Keywords"
            ])).data[0];
            ...
            meta.Keywords = meta.Keywords.filter(
                k =>
                    !data.wordsToCleanse.some(
                        w => k.toUpperCase() === w.toUpperCase()
                    )
            );

            if (data.cleanseTitle) {
                for (const w of data.wordsToCleanse) {
                    meta.Title = meta.Title.replace(
                        new RegExp(`(?:\\s?${w}|${w}\\s?)`, "ig"),
                        ""
                    );
            ...
            ep.writeMetadata(jpeg, meta, ["overwrite_original"]);

            win.send("cleanserProgressChanged", {
                progress: ++progress,
                maxProgress: maxProgress
            });
        }
